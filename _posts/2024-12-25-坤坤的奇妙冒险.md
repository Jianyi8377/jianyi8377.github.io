...
| 问题描述                                  | 解决方案                                                                 | 关键收获                                                                 |
|-------------------------------------------|--------------------------------------------------------------------------|--------------------------------------------------------------------------|
| 多线程卡顿/死锁                          | 1. 线程函数中禁用`Sleep()`，改用`clock()`循环延时；2. 用`screen`变量锁定全屏技能时的线程；3. 避免线程间共享资源竞争（如`role.bloodnow`仅在主线程修改） | 多线程同步的核心是“控制资源访问时机”，而非依赖睡眠函数；全局状态变量是轻量级同步的有效手段 |
| 画面闪烁严重                              | 1. 所有绘图操作用`BeginBatchDraw()`/`EndBatchDraw()`包裹；2. 统一用`Alldraw()`函数绘制所有元素，避免频繁局部刷新 | 双缓冲的本质是“减少屏幕刷新次数”，一次性绘制完整画面是解决闪烁的唯一途径       |
| Boss“接化发”技能反弹逻辑错误              | 1. 用`struct data* S`全局结构体记录反弹技能的初始位置与方向；2. 反弹线程`Skillback()`中独立计算轨迹，不依赖玩家位置 | 复杂技能逻辑需拆分到独立线程，避免与主线程逻辑耦合                           |
| 成就进度未保存/读取失败                   | 1. 用`fopen_s()`/`fscanf_s()`/`fprintf_s()`安全文件函数；2. 每次游戏结束（死亡/胜利）强制调用`Writecount1()-Writecount8()`；3. 增加文件打开失败的`perror()`报错 | 文件操作必须考虑“异常情况”，尤其是首次运行无文件的场景；持久化数据需确保“写入时机可靠” |
| 背景移动时敌人/道具不同步                 | 1. 在`BkMove()`函数中，同步修改敌人1/2、Boss、道具的`x`坐标；2. 限制背景移动范围（`bkX`∈[-1000, 0]），避免坐标溢出 | 大地图效果的核心是“所有对象与背景联动”，需确保移动逻辑覆盖所有相关对象         |
| 技能方向与角色朝向不同步                  | 1. 用`role.position`变量统一记录角色朝向；2. 技能释放时直接读取该变量，计算轨迹方向；3. 移动时实时更新`role.position` | 状态变量是“多模块同步”的关键，避免重复计算或硬编码方向                       |
